# Hi there 👋, I'm Mohammad Sadik

🎓 Computer Science and Engineering Student at RYM Engineering College  
💡 Passionate about Full-Stack Development, Real-Time Systems, and AI Integration  
🌱 Currently exploring Java Backend Development  
🏆 Secured 2nd Place in a competitive Web Development event

---

## 🚀 About Me
🔭 I’m working on full-stack web applications with a strong focus on backend development and scalability  
🌐 Interested in real-time systems, system design, and AI-based applications  
💬 Ask me about Node.js, REST APIs, authentication systems, and databases  
📫 Reach me at sadik.cse.rymec@gmail.com  
💼 LinkedIn | GitHub  

---

## 🛠️ Technical Skills
**Languages:** Java, JavaScript, C, C++  
**Frameworks/Technologies:** Node.js, Express.js, HTML, CSS, REST APIs, JWT, Socket.IO  
**Databases:** MongoDB, MySQL  
**Tools:** VS Code, Postman, Git, GitHub  

---

## 📌 Featured Projects

🔹 **Smart Real-Time Bus Tracking System**  
A real-time tracking system with:
- Live GPS tracking using Socket.IO  
- Geofencing using Haversine formula  
- Role-based dashboards (Admin, Driver, Student, Parent)  
- WebSocket-based real-time updates  

🔗 Live: https://track-my-bus-three.vercel.app  

---

🔹 **Food Hygiene Inspection & Reporting Platform**  
A full-stack inspection system with:
- Role-based inspection workflows  
- Cloudinary integration for image storage  
- Automated PDF report generation  
- AI-powered chatbot using Gemini API  

🔗 Live: https://food-hygiene-and-inspection-platform.onrender.com/  

---

## 📚 Relevant Coursework
- Data Structures & Algorithms  
- Operating Systems  
- Computer Networks  
- Database Management Systems  
- Object-Oriented Programming (OOP)  

---

## 🏅 Achievements
- Solved **250+ problems on LeetCode**  
- Secured **2nd Place in Web Development Competition**  
- Finalist in **Brains & Bytes-25 Hackathon**  

---

## 📊 GitHub Stats
![Sadik's GitHub stats](https://github-readme-stats.vercel.app/api?username=Sadikcserymec077&show_icons=true&theme=radical)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Sadikcserymec077&layout=compact&theme=radical)

---

⭐️ From Mohammad Sadik
